...
const Footer = () => (
    <Styled.Wrapper>
        <div className="brand"><img src="/logo.png" alt="Ashish Ranjan logo" /><span>JavaScript Core Notes</span></div>
        <nav className="links" aria-label="Social and support links">
            {links.map(([label, href, Icon]) => <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} title={label}><Icon aria-hidden="true" /></a>)}
        </nav>
